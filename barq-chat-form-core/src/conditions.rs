// Conditional logic serialization
// Serializes conditions to JSON for the data-condition attribute

use crate::schema::Condition;

/// Serialize a condition to a compact JSON string
pub fn serialize_condition(condition: &Condition) -> String {
    serde_json::to_string(condition).unwrap_or_else(|_| "{}".to_string())
}

/// Evaluate a condition (for initial visibility in Rust)
...
/// Helper to convert a JSON value to string for comparison
...
/// Helper to convert a JSON value to f64 for numeric comparisons

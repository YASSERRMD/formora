...
/// Master CSS profile - every rendered HTML element maps to exactly one field
...
impl CssProfile {
    /// Create a Bootstrap 5 CSS profile
    ...
    /// Create a Tailwind CSS v3 profile
    ...
    /// Create a custom profile with all fields empty
    pub fn custom() -> Self {
        CssProfile {
            form_wrapper: String::new(),
    ...
    pub fn merge(base: &CssProfile, overrides: &CssProfile) -> Self {
        let mut result = base.clone();

        macro_rules! merge_field {
            ($field:ident) => {
                if !overrides.$field.is_empty() {
                    result.$field = overrides.$field.clone();
                }
            };
        }

        merge_field!(form_wrapper);
        merge_field!(form_title);
        merge_field!(form_description);
        merge_field!(step_wrapper);
        merge_field!(step_title);
        merge_field!(progress_bar_wrapper);
        merge_field!(progress_bar_fill);
        merge_field!(progress_bar_label);
        merge_field!(field_group);
        merge_field!(field_label);
        merge_field!(field_required_marker);
        merge_field!(field_help_text);
        merge_field!(field_error_message);
        merge_field!(input_text);
        merge_field!(input_email);
        merge_field!(input_number);
        merge_field!(input_textarea);
        merge_field!(input_select);
        merge_field!(input_date);
        merge_field!(input_file);
        merge_field!(input_range);
        merge_field!(input_range_value_display);
        merge_field!(input_checkbox_wrapper);
        merge_field!(input_checkbox);
        merge_field!(input_checkbox_label);
        merge_field!(input_radio_wrapper);
        merge_field!(input_radio);
        merge_field!(input_radio_label);
        merge_field!(multi_select_wrapper);
        merge_field!(multi_select_tag);
        merge_field!(multi_select_tag_remove);
        merge_field!(multi_select_dropdown);
        merge_field!(multi_select_option);
        merge_field!(input_error_state);
        merge_field!(input_valid_state);
        merge_field!(field_hidden);
        merge_field!(button_submit);
        merge_field!(button_next);
        merge_field!(button_back);
        merge_field!(button_wrapper);
        merge_field!(success_wrapper);
        merge_field!(success_message);
        merge_field!(error_wrapper);
        merge_field!(error_message);

        result
    }

    /// Build a profile from a HashMap
